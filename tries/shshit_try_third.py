...
full_dataset = TubeDataset(csv_path='/home/noob/koty/new_before_last/sd_with_image_path.csv', transform=transform)
...
class TinyConvRegressionModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.features = nn.Sequential(
            ...
            nn.BatchNorm2d(8),
            nn.SiLU(),  # Good for early feature extraction
            nn.MaxPool2d(2),
            nn.Conv2d(8, 16, 3, padding=1),
            nn.BatchNorm2d(16),
            nn.GELU(),  # Smooth for deeper features
            nn.MaxPool2d(2),
        )
        self.regressor = nn.Sequential(
            nn.Flatten(),
            nn.Linear(16 * 5 * 5, 32),
            nn.Mish(),  # Very smooth for final regression
            nn.Linear(32, 1)
        )
...
criterion = nn.L1Loss()
optimizer = optim.Adam(model.parameters(), lr=0.000005, weight_decay=1e-2)
