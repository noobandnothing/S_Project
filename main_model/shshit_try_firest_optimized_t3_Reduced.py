...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun  9 12:48:37 2025
...
class LogTransformedLoss(nn.Module):
    """
    A loss function that applies a log transformation to both predictions and targets
    before computing a base loss. Useful for target variables with a wide range
    or when relative errors are more important, especially for small values.
    """
    def __init__(self, base_loss=nn.MSELoss(), epsilon=1e-6):
        """
        Initializes the LogTransformedLoss.

        Args:
            base_loss (torch.nn.Module): The base loss function to apply after
                                        log transformation (e.g., nn.MSELoss(), nn.L1Loss()).
                                        Defaults to nn.MSELoss().
            epsilon (float): A small constant added to predictions and targets
                             before taking the logarithm to prevent log(0) or log of negative values.
                             Ensure this is appropriate for the scale of your smallest target values.
        """
        super(LogTransformedLoss, self).__init__()
        self.base_loss = base_loss
        self.epsilon = epsilon

    def forward(self, pred, target):
        """
        Computes the log-transformed loss.
        ...
        """
        # Ensure predictions and targets are non-negative before log transformation
        # and add epsilon for numerical stability.
        # Clamp ensures values are at least 0 before adding epsilon
        log_pred = torch.log(torch.clamp(pred, min=0.0) + self.epsilon)
        log_target = torch.log(torch.clamp(target, min=0.0) + self.epsilon)

        # Compute the base loss on the log-transformed values
        loss = self.base_loss(log_pred, log_target)
        return loss
    
# criterion = nn.L1Loss()
criterion = LogTransformedLoss()
